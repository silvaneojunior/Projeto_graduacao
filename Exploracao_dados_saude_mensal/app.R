...
ui <- fluidPage(
    ...
    fluidRow(
            column(1,offset=1,
                   ...
                   fluidRow(radioButtons('group','Agrupamento',list('DATASUS'=F,'Reunião 30/07'=T))),
...
server <- function(input, output) {
    ...
    output$absol <- renderPlotly({
        ...
        if(input$group){
            data=ref_data
            data$Faixa_etária=as.character(data$Faixa_etária)
            data$Faixa_etária[data$Faixa_etária %in% c("15 a 19 anos","20 a 29 anos","30 a 39 anos")]='15 a 39 anos'
            data$Faixa_etária[data$Faixa_etária %in% c("40 a 49 anos","50 a 59 anos")]='40 a 59 anos'
            data$Faixa_etária[data$Faixa_etária %in% c("60 a 69 anos","70 a 79 anos","80 anos e mais")]='60 anos e mais'
            data$Faixa_etária=as.factor(data$Faixa_etária)
            data=aggregate(.~Faixa_etária+Data,data,sum)
            data[['1']]=1
        ...
        if(input$eixo_idade){
            levels(data$Faixa_etária)=values_idade
            data$Faixa_etária=as.numeric(as.character(data$Faixa_etária))
            linha=geom_line(aes(x=Faixa_etária,y=valor,color=Data))
            scale_x=scale_x_continuous('Faixa etária',breaks=unique(data$Faixa_etária),labels = labels_idade,expand=c(0,0))
            scale_color=scale_color_hue('Ano')
        }else{
            linha=geom_line(aes(x=Data,y=valor,color=Faixa_etária))
            scale_x=scale_x_date('Ano',date_breaks='1 years',expand=c(0,0),date_labels = "%Y")
