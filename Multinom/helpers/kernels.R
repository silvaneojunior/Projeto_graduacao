...
multinom_gi <- function(y,m0, C0, FF,G,D,W, pop, IC_prob=0.95){
  ...
  ss1 <- multiroot(f = model_tau0_e_tau1 , start = c(rep(0.01,r),0.03), parms = parms)
  ...
  for(t in 2:T){
    ...
    ss1 <- multiroot(f = model_tau0_e_tau1 , start = c(rep(0.01,r),0.04), parms = parms)
