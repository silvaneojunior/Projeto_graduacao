...
```{r cars, fig.height=8, fig.width=10}
...
vac_flag=rep(0,T_final)
vac_flag[indice_inter:T_final]=1
k=2
lag=12
x=c(1:lag)
mat=c()
...
M=matrix(mat,k,lag,byrow=TRUE)

extended_vac=c(rep(0,lag),vac_flag)
FF_vac=matrix(0,k,T_final)
for(t in c(1:T_final)){
  FF_vac[,t]=M%*%extended_vac[(t):(t+lag-1)]
}
...
for(i in c(1:out_var)){
  ...
  for(j in c(1:k)){
    placeholder=matrix(0,out_var,T_final)
    placeholder[i,]=FF_vac[j,]
    W=array(0,c(1,1,T_final))
    W[,,true_indice_inter]=1
    bloc_final=concat_bloco(bloc_final,
                            gera_bloco_poly(order=1,
                                            value=placeholder,
                                            name='vac_serie_' %>% paste0(i,'_',j),
                                            D=1/1,
                                            m0=0,
                                            C0=0,
                                            W=W)
...
t=c(1:lag)
for(i in c(1:k)){
  index=resultado$names[names(resultado$names)=='vac_serie_' %>% paste0(idade,'_',i)]
  beta[i]=resultado$mts[index %>% as.numeric,156]
}
poly=function(x){sum(beta*(x**t))}
fx=c()
for(i in t){fx=c(fx,poly(i))}

index=resultado$names[names(resultado$names)=='vac_serie_' %>% paste0(idade,'_',0)]
fx=c(resultado$mts[index %>% as.numeric,156],fx)

plot(c(0,t),(fx),type='l')
```
...
# Informações sobre tempo computacional

Total de modelos ajustados: $62,920$ ($12,584$ por faixa etária).
Tempo médio de ajuste de um modelo: $37.67$ milésimos de segundos (cerca de $8$ minutos para cada faixa etária).
